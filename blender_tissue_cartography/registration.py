...
# %% auto 0
__all__ = ['package_affine_transformation', 'get_inertia', 'align_by_centroid_and_intertia', 'apply_affine_to_mesh', 'procrustes',
           'icp']
...
def apply_affine_to_mesh(trafo, mesh):
    """
    Apply affine transformation to mesh. Rotate/shear and translate vertices, rotate/shear and renormalize normals,
    ...
    mesh : dict with entries vertices, faces, and optionally normals
        Mesh to be transformed
        
    Returns
    -------
    newmesh : dict with entries vertices, faces, and optionally normals
        Transformed mesh.

    """
    if trafo.shape == (3,3):
        trafo = package_affine_transformation(trafo, np.zeros(3))
    newmesh = deepcopy(mesh)
    newmesh["vertices"] = mesh["vertices"]@trafo[:3,:3].T + trafo[:3,-1]
    if "normals" in mesh:
        normals_transformed = mesh["normals"]@trafo[:3,:3].T
        newmesh["normals"] = (normals_transformed.T / np.linalg.norm(normals_transformed, axis=-1)).T
    if np.linalg.det(trafo[:3,:3]) < 0:
        newmesh["faces"] = [fc[::-1] for fc in mesh_ref["faces"]]
    return newmesh
