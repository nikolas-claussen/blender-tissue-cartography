...
# %% auto 0
__all__ = ['get_cross_section_vertices_normals', 'get_uv_layout_mask_mask', 'interpolate_per_vertex_scalar_to_UV',
...
def interpolate_per_vertex_scalar_to_UV(mesh, scalar, uv_mask=None, uv_grid_steps=256, map_back=True):
    """
    Interpolate a scalar defined per texture-vertex into the UV square.
    
    The scalar should be defined per texture-vertex. Used XXX to convert a scalar defined 
    per 3d vertex into a scalar per texture vertex.
    ...
    scalar : np.array of shape (mesh.texture_vertices.shape[0])
        Input scalar field
    ...
    interpolated : np.array of shape (uv_grid_steps, uv_grid_steps)
        Scalar field across [0,1]^2 UV grid, with uniform step size. UV positions that don't
        correspond to any value are set to np.nan.
        
    """
    matched_texture_vertices = np.copy(mesh.texture_vertices)
    if map_back:
    ...
    U, V = np.meshgrid(u, v)
    try:
        triangulation = mpl.tri.Triangulation(*matched_texture_vertices.T, mesh.texture_tris)
        interpolated = mpl.tri.LinearTriInterpolator(triangulation, scalar)(U,V).data[::-1]
        return interpolated
    except RuntimeError:
    ...
    interpolated = interpolate.griddata(matched_texture_vertices.T, scalar, (U, V), method='linear')[::-1]
    if uv_mask is not None:
        interpolated[~uv_mask,:] = np.nan
    return areas_interpolated

# %% ../nbs/02_cartographic_interpolation.ipynb 40
def get_triangle_areas_in_UV(mesh, which_area="3d", uv_mask=None, uv_grid_steps=256, map_back=True):
    """
...
def create_cartographic_projections(image, mesh, resolution, normal_offsets=(0,), uv_grid_steps=256,
    ...
    with warnings.catch_warnings(): # deal with warning if interpolator fails
        ...
        if mesh.is_triangular:
            try:
                ...
                interpolated_3d_positions = interpolate_3d_to_uv(mesh.texture_vertices,
                                                                 ...
                                                                 map_back=map_back)
                interpolated_normals = interpolate_3d_to_uv(mesh.texture_vertices, mesh.matched_normals,
                                                            mesh_triangles=mesh.texture_tris,
                                                            uv_grid_steps=uv_grid_steps,
                                                            map_back=map_back)
        ...
        interpolated_3d_positions = interpolate_3d_to_uv(mesh.texture_vertices, mesh.matched_vertices,
                                                         mesh_triangles=None,
