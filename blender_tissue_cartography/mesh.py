...
# %% auto #0
__all__ = ['flatten', 'pad_list', 'unique', 'index_else_nan', 'invert_dictionary', 'ObjMesh', 'get_flipped_uv_triangles',
           'read_other_formats_without_uv', 'split_connected_components', 'project_from_axis',
           'compute_project_from_axis_scale', 'glue_seams']

# %% ../nbs/Python library/01b_mesh.ipynb #c01ea96c
import numpy as np
...
# %% ../nbs/Python library/01b_mesh.ipynb #825202cd
...
# %% ../nbs/Python library/01b_mesh.ipynb #0364d601
class ObjMesh:
    """
    ...
    **Property methods (use like attributes)**
    ...
    def read_obj(filename):
        ...
        """
        try:
            vertices, texture_vertices, normals, faces, texture_faces, normal_faces = igl.readOBJ(filename)
            if normals.shape[0] == 0:
                obj_normals = None
            else:
                obj_normals = np.stack([igl.average_onto_vertices(vertices, faces, ni)
                                       for ni in normals[normal_faces].mean(axis=1).T], axis=-1)
...
# %% ../nbs/Python library/01b_mesh.ipynb #ab826c5a-bd43-4968-9583-ed0df1e39c28
def get_flipped_uv_triangles(texture_vertices, texture_tris):
    """
    Find UV mesh triangles which are flipped (have negative area), indicating that the UV map is not injective.

    Parameters
    ----------
    texture_vertices: np.array
        UV vertices
    texture_tri: np.array
        UV faces
    
    Returns
    -------
    flipped_inds: np.array
        Flipped faces.
    """
    uv_areas = igl.doublearea(texture_vertices, texture_tris)
    if (uv_areas > 0).mean() > 0:
        sign = 1
    else:
        sign = -1
    flipped_inds = np.where(sign*uv_areas<0)[0]
    return flipped_inds

# %% ../nbs/Python library/01b_mesh.ipynb #b075367a
...
# %% ../nbs/Python library/01b_mesh.ipynb #2437581c-7b22-4fd6-881f-ff05fa641d1c
...
# %% ../nbs/Python library/01b_mesh.ipynb #f30ed370-c1b2-422c-8d54-5a3336389a7b
...
# %% ../nbs/Python library/01b_mesh.ipynb #cffc829d
