...
# %% auto #0
__all__ = ['flatten', 'compute_per_vertex_area_distortion', 'impute_boundary_values', 'get_area_distortion_in_UV',
           'compute_per_vertex_angle_distortion', 'compute_per_face_jacobian', 'compute_per_vertex_jacobian',
           'get_metric_norm', 'get_metric_angle', 'get_induced_metric', 'tri_grad', 'get_normal_projector',
           'separate_tangential_normal', 'get_div', 'get_rot', 'get_grad_perp']

# %% ../nbs/Python library/03_differential_geometry.ipynb #59151c59
...
# %% ../nbs/Python library/03_differential_geometry.ipynb #e20bd148-25f0-4e44-8b81-b879bd0c6779
...
# %% ../nbs/Python library/03_differential_geometry.ipynb #f6320083
def compute_per_vertex_area_distortion(source_vertices, source_faces, target_vertices, target_faces,
    ...
    if evaluate_at == 'target':
        area_ratio_at_vertices = igl.average_onto_vertices(target_vertices, target_faces, area_ratio)
    elif evaluate_at == 'source':
        area_ratio_at_vertices = igl.average_onto_vertices(source_vertices, source_faces, area_ratio)
    return area_ratio_at_vertices

# %% ../nbs/Python library/03_differential_geometry.ipynb #2fb1f8de-8bfe-48b5-8835-83183a90a5d1
def impute_boundary_values(per_vertex_field, tris, fill_value=np.nan):
    """
    Replace values of per_vertex_field at boundary vertices by average of its non-boundary neighbors.
    If there are no non-boundary neighbors, use fill_value.
    """
    boundary_verts = flatten(igl.boundary_loop_all(tris))
    adjacency_list = igl.adjacency_list(tris)
    imputed_field = np.copy(per_vertex_field)

    for v in boundary_verts:
        nghbs = [n for n in adjacency_list[v] if not n in boundary_verts]
        if nghbs:
            imputed_field[v] = imputed_field[nghbs].mean(axis=0)
        else:
            imputed_field[v] = fill_value
    return imputed_field

# %% ../nbs/Python library/03_differential_geometry.ipynb #ebe5b7cd
def get_area_distortion_in_UV(mesh, uv_grid_steps=1024, map_back=True):
    ...
    distortion = impute_boundary_values(distortion, mesh.texture_tris, fill_value=1)
...
# %% ../nbs/Python library/03_differential_geometry.ipynb #1d06b050
def compute_per_vertex_angle_distortion(source_vertices, source_faces, target_vertices, target_faces,
    ...
    if evaluate_at == 'target':
        angle_error_at_vertices = igl.average_onto_vertices(target_vertices, target_faces, angle_error)
    elif evaluate_at == 'source':
        angle_error_at_vertices = igl.average_onto_vertices(source_vertices, source_faces, angle_error)
    return angle_error_at_vertices

# %% ../nbs/Python library/03_differential_geometry.ipynb #fa840ee7
...
def compute_per_vertex_jacobian(source_vertices, source_faces, target_vertices, target_faces,
    ...
    jac_per_vertices = np.stack([igl.average_onto_vertices(source_vertices, source_faces, x)
                                 for x in jac_per_face.reshape((jac_per_face.shape[0],-1)).T], axis=-1)
    jac_per_vertices = jac_per_vertices.reshape((-1,)+jac_per_face.shape[1:])
    return jac_per_vertices

# %% ../nbs/Python library/03_differential_geometry.ipynb #5dfb5f01
...
# %% ../nbs/Python library/03_differential_geometry.ipynb #92aa667d
def tri_grad(field, vertices, faces, grad_matrix=None):
    ...
    grad_field = np.stack([[igl.average_onto_vertices(vertices, faces, grad_field[:, i, j])
                            for i in range(grad_field.shape[1])] for j in range(grad_field.shape[2])]).T
    # finally, reshape into original shape
    grad_field = grad_field.reshape(grad_field.shape[:2] + index_shape)
    return grad_field

# %% ../nbs/Python library/03_differential_geometry.ipynb #edf2de3d
...
# %% ../nbs/Python library/03_differential_geometry.ipynb #1d85afe3-3dff-4caf-a8de-190eba81774a
...
# %% ../nbs/Python library/03_differential_geometry.ipynb #033e0d24
